...
import { fakeTask } from '../fixtures/fake-tasks';
import { AiGeneratorPage } from '../pages/AiGeneratorPage';
import { TaskListPage } from '../pages/TaskListPage';

test.describe('RF-05 / RF-06 — Geração por IA (API mockada)', () => {
  test('RF-05: com objetivo vazio, gerar não adiciona tarefas', async ({ page, passo }) => {
    let before = 0;
    await passo('Abrir app e contar itens', async () => {
      await page.goto('/');
      const list = new TaskListPage(page);
      await list.expectLoaded();
      before = await page.getByTestId('task-item').count();
    });
    await passo('Clicar em gerar com objetivo vazio', async () => {
  ...
  test('RF-05: indicador de carregamento durante POST /ai/generate', async ({ page, passo }) => {
    await passo('Registrar mock com atraso na API', async () => {
    ...
    await passo('Disparar geração e ver Carregando...', async () => {
      await page.goto('/');
      const ai = new AiGeneratorPage(page);
  ...
  test('RF-05: subtarefas mockadas aparecem na lista com badge IA', async ({ page, passo }) => {
    const t1 = `[e2e] IA badge A ${Date.now()}`;
    const t2 = `[e2e] IA badge B ${Date.now()}`;

    await passo('Mock de resposta IA e fluxo na UI', async () => {
      await page.route(`${apiBaseURL}/ai/generate`, async (route) => {
        if (route.request().method() !== 'POST') {
          await route.continue();
          return;
  ...
  test('RF-06: campo de API Key está acessível e aceita valor antes da geração mockada', async ({
    ...
    await passo('Mock IA e preencher chave + objetivo', async () => {
      await page.route(`${apiBaseURL}/ai/generate`, async (route) => {
        if (route.request().method() !== 'POST') {
          await route.continue();
          return;
