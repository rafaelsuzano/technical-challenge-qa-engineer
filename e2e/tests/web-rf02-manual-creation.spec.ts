...
test.describe('RF-02 — Criação manual (detalhes de UX)', () => {
  test('após criar tarefa com sucesso o campo de título é limpo', async ({ page, passo }) => {
    const title = `[e2e] RF02 limpar ${Date.now()}`;
    await passo('Criar tarefa e conferir campo limpo', async () => {
      await page.goto('/');
  ...
  test('RF-02: confirmar criação com Enter no campo de título', async ({ page, passo }) => {
    const title = `[e2e] RF02 enter ${Date.now()}`;
    await passo('Preencher título e enviar com Enter', async () => {
      await page.goto('/');
      const list = new TaskListPage(page);
  ...
  test('várias tarefas manuais aparecem na lista', async ({ page, passo }) => {
    await passo('Criar três tarefas e validar contagem', async () => {
      await page.goto('/');
      const form = new TaskFormPage(page);
      const list = new TaskListPage(page);
